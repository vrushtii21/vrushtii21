...
WHERE
    hiredate < 1980 - 12 - 31;
SELECT 
    empno, ename, SUM(sal * 12) AS annsal
FROM
    emp
...
ORDER BY job DESCst the emps Who Annual sal ranging from 22000 and 45000.
select ename,
       empno,
...
BEGIN
    IF OLD.sal  <> NEW.sal THEN
        INSERT INTO emp_log (empno, log_date, new_salary, Action)
	    values(new.empno, current_date, new.sal, 'new salary');
	end if;
end $$
delimiter ;
